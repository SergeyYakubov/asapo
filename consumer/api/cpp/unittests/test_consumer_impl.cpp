...
TEST_F(ConsumerImplTests, GetLastUsesCorrectUri) {
    ...
    EXPECT_CALL(mock_http_client,
                Get_t(expected_broker_api + "/beamtime/beamtime_id/" + expected_data_source_encoded + "/"+ expected_stream_encoded+"/0/last?token="
                          + expected_token, _,
                      _)).WillOnce(DoAll(
        SetArgPointee<1>(HttpCode::OK),
        SetArgPointee<2>(nullptr),
...
TEST_F(ConsumerImplTests, GetMessageReturnsNoDataAfterTimeoutEvenIfOtherErrorOccured) {
    ...
    EXPECT_CALL(mock_http_client, Get_t(expected_broker_api + "/beamtime/beamtime_id/" + expected_data_source_encoded + "/stream/0/"
...
TEST_F(ConsumerImplTests, GetNextDatasetUsesCorrectUri) {
    ...
    EXPECT_CALL(mock_http_client, Get_t(expected_broker_api + "/beamtime/beamtime_id/" + expected_data_source_encoded + "/stream/" +
...
TEST_F(ConsumerImplTests, DeleteStreamUsesCorrectUri) {
    ...
    EXPECT_CALL(mock_http_client, Post_t(expected_broker_api + "/beamtime/beamtime_id/" + expected_data_source_encoded + "/"+expected_stream_encoded+"/delete"
