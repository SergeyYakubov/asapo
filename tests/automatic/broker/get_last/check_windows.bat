...
C:\Curl\curl.exe -v  --silent 127.0.0.1:5005/database/data/stream/default/%groupid%/last?token=%token% --stderr - | findstr /c:\"_id\":2  || goto :error
C:\Curl\curl.exe -v  --silent 127.0.0.1:5005/database/data/stream/default/%groupid%/last?token=%token% --stderr - | findstr /c:\"_id\":2  || goto :error

echo db.data_default.insert({"_id":3}) | %mongo_exe% %database_name%  || goto :error
C:\Curl\curl.exe -v  --silent 127.0.0.1:5005/database/data/stream/default/%groupid%/last?token=%token% --stderr - | findstr /c:\"_id\":3  || goto :error
